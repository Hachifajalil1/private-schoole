...
fun HomeAdmins(innerPadding: PaddingValues) {
    ...
    // عرض تفاصيل الادمن المحدد
    selectedAdmin?.let { admin ->
        AdminDetails(
            admin = admin,
            innerPadding = innerPadding,
            onDeleteClick = { deleteUserData(admin.uid)
                selectedAdmin = null
                loadState = LoadState.Idle },
            onBackClick = { selectedAdmin = null
                loadState = LoadState.Idle },
        )
    }

    // التأكد من أن البيانات لم تُحمل بعد
            ...
            Firebase.database.reference.child("users").child("admins")
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    ...
                    override fun onCancelled(error: DatabaseError) {
                        ...
                        loadState = LoadState.Loaded // يمكن التعامل مع هذه الحالة بشكل مختلف
                    }
                })
        }
    }
